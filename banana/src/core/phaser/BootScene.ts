...
preload() {
  ...
  for (let i = 1; i <= 6; i++) {
    ...
    this.load.image(key, `../src/assets/backgrounds/${key}.png`);
  }

  // 오른쪽 달리기 → png
  ['run_r_00','run_r_01','run_r_02','run_r_03'].forEach(f => {
    this.load.image(`banana_${f}`, `../src/assets/sprites/banana/${f}.png`);
  });

  // 왼쪽 달리기 → jpg
  ['run_l_00','run_l_01','run_l_02','run_l_03'].forEach(f => {
    this.load.image(`banana_${f}`, `../src/assets/sprites/banana/${f}.jpg`);
  });

  // 나머지 공용
  ['jump_r','land_r','jump_l','land_l','explode'].forEach(f => {
    this.load.image(`banana_${f}`, `../src/assets/sprites/banana/${f}.png`);
  });
}
